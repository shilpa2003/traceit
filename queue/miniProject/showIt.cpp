...
int i=0,count=0,hd1=0,xmax,ymax,ai=0,si=0,lines=0,wd=9,st=0,prevVal=-1,currentVal=0,factor=180,qi=0;
...
void storeVarType(int j,char ch1){
	...
	case 's': strcpy(hold1[j].type,"stack");
			break;
	case 'q': strcpy(hold1[j].type,"queue");
			break;
}
}
...
void showArrIndex(int x,int y,int id,int ai){
	...
	cout<<"@@@@@@@@@@@@@@@@@@@@@ index : "<<hold1[id].arrIndex[ai]<<endl;
	if(hold1[st].queueFalg==1){
		if(strcmp(hold1[id].varsValues[1],hold1[id].arrIndex[ai])==0 && len>0){
		k=0;
		//x=left1-60;
		glRasterPos2f(x,y+70);
		cout<<"*********************************************Rear*************************************\n"<<hold1[id].vars[1]<<endl;
		...
		if(strcmp(hold1[id].varsValues[2],hold1[id].arrIndex[ai])==0){
		k=0;
		//x=left1-60;
		glRasterPos2f(x,y+80);
		cout<<"*********************************************Front*************************************\n"<<hold1[id].vars[2]<<endl;
		len=strlen(hold1[id].vars[2]);
		while(k<=len){
			glutBitmapCharacter(GLUT_BITMAP_9_BY_15,hold1[id].vars[2][k]);
			k++;
		}
		}
...
void drawQueueRec(int arrSize,int id){
	int k=0;
	boxes=0;
	left1=100;
	right1=180;
...
void processQueue(int k){
	...
	if(ch==':'){
		...
		if(flag==1){
			//si-=1;
			qi+=1;
			//strcpy(hold1[st].varsValues[1],hold1[k].arrIndex[ij-1]);
			strcpy(hold1[st].varsValues[2],hold1[k].arrIndex[qi]);
			cout<<" duplicate  : "<<hold1[k].arrIndex[ai]<<endl;
			hold1[k].arrIndex[ij][0]='\0';
			strcpy(hold1[k].varsValues[1],hold1[k].arrIndex[si-1]);
		}
		else{
			strcpy(hold1[k].arrIndex[si],temp);
		}
	...
	if(ch==';'){
		...
		if(flag==1){
			si-=1;
			hold1[k].arrValues[ij][0]='\0';
			strcpy(hold1[st].mesg,"-dequeue");
			flag=0;
		}
		else{
			strcpy(hold1[k].arrValues[si],temp);
			strcpy(hold1[st].mesg,"-enqueue");
		}
		//count+=1;
		i=0;
		file.get(ch);
		...
		if(ch!=':' && ch!=';'){
			...
			if(type!='~'){
				...
				if(k<=hd1){
					...
					else if(strcmp(hold1[k].type,"queue")==0){
						processQueue(k);
					}
					else{
					count+=1;
